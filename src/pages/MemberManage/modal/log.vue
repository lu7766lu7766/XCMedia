<template>
  <detail :title="`登入纪录-${account}`" :hide-footer="true">
...
export default {
  ...
  methods: {
    async getHistorys (_data) {
      const data = Object.assign({}, this.paginate, { id: _data.id })
      const { data: historys } = await this.$thisApi.getLoginHistorys(data)
